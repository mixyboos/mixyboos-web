...
import type { CreateMixModel, MixModel } from '@/lib/models/mix'
...
const getMixesFeed = async (): Promise<Array<MixModel>> => {
  try {
...
const getMyMixes = async (): Promise<Array<MixModel>> => {
  try {
...
const getUserMixes = async (user: string): Promise<Array<MixModel>> => {
  try {
