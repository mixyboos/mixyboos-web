...
const useAudioProcessingStatus = (options: UseAudioProcessingStatusOptions = {}) => {
  const { mixId } = options
  ...
  useEffect(() => {
    ...
    connection.start().then(() => {
      logger.debug(
        { context: 'Signalr', action: 'useAudioProcessingStatus', connection, mixId },
        'Connected',
      )
      ...
      connection.on('ConversionProgress', (receivedMixId: string, value: number) => {
        if (!mixId || mixId === receivedMixId) {
          logger.debug(
            { context: 'Signalr', action: 'ConversionProgress', receivedMixId, value },
            value.toString(),
          )
      ...
      connection.on('ConversionFailed', (receivedMixId: string, errorMessage?: string) => {
        if (!mixId || mixId === receivedMixId) {
          logger.debug({
            context: 'Signalr',
    ...
    return () => {
      connection.stop().catch((err) => {
        logger.error({ context: 'Signalr', action: 'stop', error: err }, 'Error stopping connection')
      })
    }
  }, [mixId])
